...
//	freeDoubleCharArray(11, info);//deeply free
    fread(routeP, sizeof(route), 1, fText);
    if (feof(fText)) {
		return NULL;
	}
	routeP->next = NULL;
	...
	while (1) {
        ...
        fread(tempP, sizeof(route), 1, fText);
		tempP->firstSite = NULL;
...
//        return NULL;
//    }
    fread(siteP, sizeof(site), 1, fText);
    if(feof(fText)){
        return NULL;
    }
    siteP->carHeadP = NULL;
...
//	freeDoubleCharArray(9, info);
	...
	while (1) {
        ...
        fread(tempP, sizeof(site), 1, fText);
//		char StrLine[200];
//		fgets(StrLine, 200, fText);
		if (feof(fText)) {
			free(tempP);
...
//返回带有货物信息的车辆链表（因为共存于同一个二进制文件中，无法拆分
...
//    }
    ...
    if (carP->good != NULL){
        ...
        fread(goodP, sizeof(good), 1, fText);
        carP->good = goodP;
    }
...
//	freeDoubleCharArray(4, info);
	...
	while (1) {
        ...
        fread(tempP, sizeof(car), 1, fText);
...
/* 获得某位置结点的地址，让外部进行修改, 失败则返回空指针, 从0记起*/
...
site* DelSitePos(site *HeadP, int pos) {
	...
	if (pHead == NULL){//pos超界，寻找不到对应的结点
	    printf("输入指令错误\n");
        return HeadP;
	}
	if (i == 0) {//即删除头结点
		site * returnP = pHead->next;//返回第二个结点
		//deeply free
		carP = pHead->carHeadP;
		while(carP != NULL){
            carPp = carP->next;
