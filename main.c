...
void addRoute(route * routeHeadP) {
	...
	if (newRouteP != NULL) {
		...
		printSomeSpace(60);printf("请输入总公里数:");
		float fMiles = 0;
		scanf("%f%*c", &fMiles);
		newRouteP->miles = fMiles;
		printSomeSpace(60);printf("请输入总耗时:");
		float fPeriod = 0;
		scanf("%f%*c", &fPeriod);
...
int changeSite(site * siteSpecific, int notFirst, site * siteHeadP) {
	...
	if (notFirst){
	    ...
	    printf("%60s1.站点编号\n%60s2.站点名称\n%60s3.经过本站点的固定路线编号\n%60s4.停留耗时\n%60s5.与上一站交通耗时\n\n%60s请选择你要修改的属性:",space,space,space,space,space);
	}
	int choose = 0;
	scanf("%d%*c", &choose);
...
void printSitePage(route *routeP, route * routeHeadP) {
	...
	while (inSitePage) {
		...
		site* siteP = siteHeadP;
		system("date /T");
		printSomeG(71);printf("路线编号：%6s 的所有站点信息", siteHeadP->routeID);printSomeG(71);printf("\n");//title
